...
图\ref{3-5}此页面提供了我常用的几个资源小站，在这个页面中首次尝试了JavaScript代码，以显示鼠标悬停时的悬浮显示效果，既与前几个页面风格保持一致，同时对每个方框内的资源做了简要介绍。
...
\begin{shaded*}
	\begin{alg}{不同的部分}
		\label{alg:1}
		\begin{algorithmic}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{document.addEventListener('DOMContentLoaded', () => \{ }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{const tooltip = document.createElement('div');}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.className = 'tooltip';}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{document.body.appendChild(tooltip); }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{document.querySelectorAll('.link-box').forEach(box => \{ }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{box.addEventListener('mouseenter', (e) => \{ }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.innerText = e.currentTarget.getAttribute('data-description');}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.style.opacity = '1';}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{\});}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{box.addEventListener('mousemove', (e) => \{ }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.style.left = `${e.pageX + 15}px`; }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.style.top = `${e.pageY + 15}px`; }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{\});}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{box.addEventListener('mouseleave', () => \{ }
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{tooltip.style.opacity = '0';}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{\});}
			\State \ \ \ \ \ \ \ \ \ \ \ \ \textbf{\});}
		\end{algorithmic}
	\end{alg}
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.20]{images/3-5.png}
		\caption{分页面举例}
		\label{fig3-5}
	\end{center}
\end{figure}
