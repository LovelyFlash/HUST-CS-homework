  ...
  <div class="container">
        ...
        <el-dropdown-menu>
          <el-dropdown-item>
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>TL：图灵</el-dropdown-item>
                  <el-dropdown-item>CS：计科</el-dropdown-item>
                  <el-dropdown-item>BD：大数据</el-dropdown-item>
                  <el-dropdown-item>BSB：本硕博/启明实验班</el-dropdown-item>
                  <el-dropdown-item>IOT：物联网</el-dropdown-item>
                  <el-dropdown-item>ZY：卓越/创新实验班</el-dropdown-item>
                  <el-dropdown-item>IST：智能数据与科学</el-dropdown-item>
                  <!-- 可根据实际情况补充 -->
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          ...
          <el-dropdown-item>
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item
                    >第一个1是第一学年，第二个1是第一学期，以此类推</el-dropdown-item
                  >
                  <!-- 可根据实际情况补充 -->
                </el-dropdown-menu>
              </template>
    ...
    <el-row>
      <el-col :span="8">
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">上传的文件列表如下：</div>
          </template>
        </el-upload>
    ...
    <el-row style="margin-top: 20px">
      ...
      <el-col :span="6">
        ...
        <el-button type="danger" color="red" @click="flushALLData"
          >删除全部数据</el-button
        >
      </el-col>
    </el-row>
    ...
    <el-row style="color: #1f2f3d; font-size: 16px">
      ...
      <el-col :span="3" style="text-align: center"
        ><el-input
          style="width: 75%"
          placeholder="无,则不填"
          v-model="totalClassNum"
        ></el-input
      ></el-col>
      <el-col :span="2" style="text-align: center"
        ><el-button type="info" @click="reset">复位</el-button></el-col
      >
      <el-col :span="2" style="text-align: center"
        ><el-button type="primary" @click="commitExcellent"
          >上传</el-button
        ></el-col
      >
      <el-col :span="1"></el-col>
      <el-col :span="4" style="text-align: right"
        ><el-button type="danger" color="red" @click="flushALL"
          >删除全部数据</el-button
        ></el-col
      >
    </el-row>
    ...
    <el-row style="color: #1f2f3d; font-size: 16px">
      ...
      <el-col :span="4" style="text-align: center"
        ><el-input
          style="width: 75%"
          placeholder="填浮点数"
          v-model="requiredCreditExcludePublicElective"
        ></el-input
      ></el-col>
      <el-col :span="4" style="text-align: center"
        ><el-input
          style="width: 75%"
          placeholder="填浮点数"
          v-model="requiredCreditIncludePublicElective"
        ></el-input
      ></el-col>
      <el-col :span="2" style="text-align: center"
        ><el-button type="info" @click="resetInfo">复位</el-button></el-col
      >
      <el-col :span="2" style="text-align: center"
        ><el-button type="primary" @click="setStudentInfo"
          >上传</el-button
        ></el-col
      >
    </el-row>
...
import {
  dataReduction,
  dataReductionPersonal,
  uploadExcellentClassData,
  flushALLExcellent,
  downloadCourseFile,
  courseCalculate,
  scoreCalculate,
...
import { ArrowDown } from "@element-plus/icons-vue"; //xyk
...
const getMajorList = () => {
  ...
  for (var i in majorArray) {
    majorList.value.push({
      label: majorMap.get(majorArray[i])!,
      value: majorArray[i],
    });
  }
...
const setStudentInfo = () => {
  if (
    ...
    requiredCreditExcludePublicElective.value == "" ||
    requiredCreditIncludePublicElective.value == "" ||
    Number(redRate.value) < 0.0 ||
    Number(redRate.value) > 1.0 ||
    Number(yellowRate.value) < 0.0 ||
    Number(yellowRate.value) > 1.0 ||
    Number(requiredCreditExcludePublicElective.value) < 0.0 ||
    Number(requiredCreditIncludePublicElective.value) < 0.0
  ) {
    ElMessage.warning({ message: "格式错误" });
    return;
  } else {
    setStudentInfoConfig({
      grade: gradeForInfo.value,
      major: major.value,
      redRate: Number(redRate.value),
      yellowRate: Number(yellowRate.value),
...
const courseCalc = () => {
  ElMessageBox.confirm("大概需要2-3分钟!", "数据整理(课程分类)", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  }).then(() => {
    courseCalculate().then((res) => {
      console.log(res);
    });
    ElMessage({
      type: "info",
...
const scoreCalc = () => {
  ElMessageBox.confirm("大概需要1-2分钟!", "数据整理(平均分)", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  }).then(() => {
    scoreCalculate().then((res) => {
      console.log(res);
    });
    ElMessage({
      type: "info",
...
const flushALL = () => {
  ElMessageBox.confirm("是否删除全部优良学风班数据?", "删除数据", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  }).then(() => {
...
const commitExcellent = () => {
  ...
  if (year1.value != "" && /^\d+$/.test(year1.value))
    uploadList.push({
      ...
      year: Number(grade.value) + 2001,
      excellentClassNum: Number(year1.value),
      totalClassNum: Number(totalClassNum.value),
    });
  else if (year1.value != "") {
  ...
  if (year2.value != "" && /^\d+$/.test(year2.value))
    uploadList.push({
      grade: Number(grade.value),
      year: Number(grade.value) + 2002,
      excellentClassNum: Number(year2.value),
      totalClassNum: Number(totalClassNum.value),
    });
  else if (year2.value != "") {
  ...
  if (year3.value != "" && /^\d+$/.test(year3.value))
    uploadList.push({
      grade: Number(grade.value),
      year: Number(grade.value) + 2003,
      excellentClassNum: Number(year3.value),
      totalClassNum: Number(totalClassNum.value),
    });
  else if (year3.value != "") {
    ...
    .then(async () => {
      ...
      for (var i in uploadList) {
        await uploadExcellentClassData(uploadList[i]).then((res) => {
          console.log(res);
          if (res.data.code == 200) success += 1;
          else failed += 1;
        });
      }
...
const submitUpload = () => {
  ...
  ElMessageBox.confirm(fileNames, "上传文件", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  })
...
const dataReduct = () => {
  ElMessageBox.confirm("大概需要3-5分钟!", "数据整理(全部)", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  }).then(() => {
    dataReduction().then((res) => {
      console.log(res);
    });
    ElMessage({
      type: "info",
...
const dataReductPersonal = () => {
  ElMessageBox.confirm("大概需要3-5分钟!", "数据整理(仅个人)", {
    confirmButtonText: "确认",
    cancelButtonText: "取消",
    dangerouslyUseHTMLString: true,
  }).then(() => {
    dataReductionPersonal().then((res) => {
      console.log(res);
    });
    ElMessage({
      type: "info",
