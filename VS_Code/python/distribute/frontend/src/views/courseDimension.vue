  ...
  <div>
    ...
    <div class="container">
      <div class="handle-box">
        ...
        >
        <!-- xyk -->
        <el-button type="success" @click="exportYear(radioCourse)">
          按学年导出
        </el-button>
      </div>
...
// exportyear函数为xyk编写，功能为按学年导出挂科名单，有问题直接删
const exportYear = (course: string) => {
  // 获取当前学年
  const year = term.substring(0, 1);
  // 固定为第二学期
  const semester = "2";
  if (!course) {
    ElMessage.warning("请先选择课程后再导出！");
    return;
  }
  ...
  downloadCoursesData({
    ...
    if (res.status == 200) {
      fileDownload(res.data, `failed_students_year_${year}_${course}.xlsx`);
      ElMessage.success({
        message: "文件下载成功！",
        type: "success",
      });
